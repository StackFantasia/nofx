...
本版本在原项目基础上进行了深度优化，核心理念是通过改进 AI 的推理能力和决策质量来提升交易胜率。主要改进包括：
...
**全新工作流程：**
1. **配置 AI 模型**：通过网页界面添加您的 DeepSeek/Qwen API 密钥
2. **配置交易所**：设置 Binance/Hyperliquid API 凭证
3. **创建交易员**：将任何 AI 模型与任何交易所结合，创建自定义交易员
4. **监控与控制**：启动/停止交易员并实时监控其性能

**本次更新的意义**
- 🎯 **用户友好**：无需再编辑 JSON 文件或重启服务器
- 🔧 **灵活**：可将不同的 AI 模型与不同的交易所进行混合搭配
- 📊 **可扩展**：可创建无限数量的交易员组合
- 🔒 **安全**：采用数据库存储并进行妥善的数据管理

请参阅[快速入门](#-quick-start)了解新的设置流程！
...
#### **步骤 2：配置交易所**
1. 点击“交易所配置”按钮
2. 启用 Binance 或 Hyperliquid（或两者都启用）
3. 输入您的 API 凭证
4. 保存配置

#### **步骤 3：创建交易员**
1. 点击“创建交易员”按钮
2. 选择一个 AI 模型（必须先进行配置）
3. 选择一个交易所（必须先进行配置）
4. 设置初始余额和交易员名称
5. 创建交易员
...
**NOFX 支持 Hyperliquid**——一个高性能的去中心化永续期货交易所！

**为什么选择 Hyperliquid？**
- 🚀 **高性能**：在 L1 区块链上实现闪电般的执行速度
- 💰 **低手续费**：极具竞争力的挂单/吃单手续费
- 🔐 **非托管**：您的私钥，您的代币
- 🌐 **无需 KYC**：匿名交易
- 💎 **超强流动性**：机构级订单簿
...
**⚠️安全警告**：切勿分享您的私钥！请使用专用钱包进行交易，不要使用您的主钱包。
...
**📖 有关详细设置说明，请参阅**：[Aster 集成指南](ASTER_INTEGRATION.md)
